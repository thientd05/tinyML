...
This is THE reported training path (single-seed `python -m src.models.<fam>` is only the
fast path). The per-beat operating point (precision @ recall 0.95 on DS2) is high-variance
for the raw-beat nets — a single run swings ~0.05 just from the seed, which previously
produced a non-reproducible "winner". This trains every (family, size) over several seeds,
keeps the SEED-MEDIAN checkpoint as the representative (so export/parity + cost stay
consistent), and writes the per-seed mean/std into each model/<family>_<size>_metrics.json
under "seeds". `src.analysis.analyze` then selects winners on the MEAN and reports mean±std.

Library entry point: `run_multiseed(seeds, families)`. The CLI wrapper is tools/multiseed.py.
"""
...
DEFAULT_FAMILIES = ["rf", "xgb", "svm", "cnn", "lstm", "crnn"]
...
def run_multiseed(seeds: list[int], families: list[str] | None = None) -> None:
    """Train every (family, size) over `seeds`, saving the seed-median checkpoint + mean/std."""
    families = families or DEFAULT_FAMILIES
    print(f"[multiseed] seeds={seeds} families={families}")
    for fam in families:
        if fam in RUNNERS:
            RUNNERS[fam](seeds)
        elif fam in NETS:
