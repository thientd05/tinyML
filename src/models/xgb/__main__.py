"""CLI entrypoint: train/load the XGBoost capacity sweep and dump per-variant metrics."""
...
from src.models.xgb import EXT, MODEL_NAME, SWEEP, build, scores, train
...
def main() -> None:
    ...
    print(f"[xgb] === sweep (DS2; threshold tuned on DS1-val to recall>={TARGET_RECALL}) ===")
    for size, n_est, depth in SWEEP:
        path = model_path(MODEL_NAME, size, EXT)
        if path.exists() and not args.retrain:
            model = joblib.load(path)
            print(f"[xgb] loaded {path.name}")
        else:
            print(f"[xgb] training {size} (n_estimators={n_est}, max_depth={depth}) ...")
        ...
        rec.update(family=MODEL_NAME, size=size, cost=cost.xgb_cost(model))
