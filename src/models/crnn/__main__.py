"""CLI entrypoint: train/load the CRNN capacity sweep and dump per-variant metrics."""
...
from src.models.crnn import EXT, MODEL_NAME, SWEEP, ECGCRNN, scores, train_one
...
def main() -> None:
    ...
    print(f"[crnn] device={device}")

    # Same raw-beat input as the CNN (patient-pure DS1-val for checkpoint + threshold).
    X_tr, y_tr, X_val, y_val, X_test, y_test = build_dataset(mode="raw")
    cw = torch.from_numpy(class_weights(y_tr))
    print(f"[crnn] train={X_tr.shape} val={X_val.shape} test={X_test.shape}  "
          f"class_weights={cw.tolist()}")

    print(f"[crnn] === sweep (DS2; threshold tuned on DS1-val to recall>={TARGET_RECALL}) ===")
    for size, channels, hidden in SWEEP:
        path = model_path(MODEL_NAME, size, EXT)
        model = ECGCRNN(channels, hidden)
        if path.exists() and not args.retrain:
            model.load_state_dict(torch.load(path, map_location="cpu"))
            print(f"[crnn] loaded {path.name}")
        else:
            print(f"[crnn] training {size} (channels={channels}, hidden={hidden}) ...")
            train_one(model, X_tr, y_tr, X_val, y_val, device=device,
                      epochs=args.epochs, batch_size=args.batch_size,
                      lr=args.lr, weight_decay=args.weight_decay,
                      class_weight_t=cw)
            torch.save(model.state_dict(), path)
            print(f"[crnn] saved -> {path}")
        ...
        rec.update(family=MODEL_NAME, size=size,
                   cost=cost.crnn_cost(channels, hidden, BEAT_LEN))
